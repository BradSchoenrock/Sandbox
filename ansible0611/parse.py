...
def main():
    ...
    with open(''.join([i for i in inFile[:-8] if not i.isdigit()])+"_Stats.txt",'w') as f:
        sys.stdout = f
        print (myData.describe())
    ...
    plt.xlabel("time(s)")
    plt.ylabel("RSS(mb)")
    plt.title("size vs time " + ''.join([i for i in inFile[:-8] if not i.isdigit()]))
    plt.tight_layout()
    plt.savefig('/home/bschoenrock/git/OldLaTeXDouments/AV2_16SessionSize/figures/'+inFile[:-8]+'_Mem.png')
    #plt.show()
    plt.close()
    ...
    plt.xlabel("time(s)")
    plt.ylabel("CPU%")
    plt.title("CPU vs time " + ''.join([i for i in inFile[:-8] if not i.isdigit()]))
    plt.tight_layout()
    plt.savefig('/home/bschoenrock/git/OldLaTeXDouments/AV2_16SessionSize/figures/'+inFile[:-8]+'_CPU.png')
    #plt.show()
    plt.close()
